...
(function () {
    logger.info("Augment context for: {}", security.authenticationId);
    ...
    if (typeof properties.managedUserLink === "string" && properties.managedUserLink.length) {
        ...
        managedUserId = openidm.query("repo/link", {
            "_queryId": "links-for-secondId",
            "linkType": properties.managedUserLink,
            "secondId" : userDetail.result[0]._id
        });
        ...
        managedUser = openidm.read("managed/user/" + managedUserId.result[0].firstId);
